...
func TestAddNote(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedBody:   `{"error":"failed to add note"}` + "\n",
			reqBody:        note,
		},
	}
...
func TestGetNotes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedBody:   `{"error":"failed to get notes"}` + "\n",
		},
	}
...
func TestUpdateNote(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockReturn:     nil,
			expectedStatus: http.StatusAccepted,
			expectedBody:   `{"status":"update accepted"}`,
		...
		},
		{
			...
			mockReturn:     nil,
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `{"error":"invalid UUID length: 0"}` + "\n",
			noteID:         "",
			reqBody:        note,
			expectCall:     false,
		},
		{
			...
			mockReturn:     nil,
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `{"error":"invalid UUID length: 10"}` + "\n",
			noteID:         "123a45test",
			reqBody:        note,
			expectCall:     false,
		},
...
func TestDelNote(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedStatus: http.StatusInternalServerError,
			expectedBody:   `{"error":"delete failed"}` + "\n",
			noteID:         validUUID.String(),
			expectCall:     true,
		},
		{
