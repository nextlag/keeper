...
func (api *ClientAPI) addEntity(entity interface{}, accessToken, endpoint string) error {
	client := resty.New()
...
func (api *ClientAPI) getEntities(entity interface{}, accessToken, endpoint string) error {
	client := resty.New()
	client.SetAuthToken(accessToken)
	resp, err := client.R().
		SetResult(entity).
