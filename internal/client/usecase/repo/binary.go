...
func (r *Repo) SaveBinaries(binaries []entity.Binary) error {
	if len(binaries) == 0 {
		return nil
	}
	userID := r.getUserID()
...
func (r *Repo) AddBinary(binary *entity.Binary) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		for _, meta := range binary.Meta {
			metaForBinary := models.MetaBinary{
				Name:     meta.Name,
				Value:    meta.Value,
				BinaryID: binaryForSaving.ID,
				ID:       meta.ID,
			}
...
func (r *Repo) GetBinaryByID(binaryID uuid.UUID) (binary entity.Binary, err error) {
...
func (r *Repo) DelBinary(binaryID uuid.UUID) error {
	return r.db.Unscoped().Delete(&models.Binary{}, binaryID).Error
}
